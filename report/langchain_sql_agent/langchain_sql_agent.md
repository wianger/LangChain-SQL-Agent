...
查询处理流水线实现了一种结构化检索策略，通过利用三层存储层次结构，在检索准确性与上下文窗口成本之间取得平衡。

```mermaid
flowchart TD
    Q([用户查询]) --> V[语义意图提取<br>LLM解析查询意图<br>识别领域关键词与实体]
    V --> C1[L0目录扫描<br>将表名与提取的<br>关键词和实体进行匹配]
    C1 --> R1{识别到<br>相关表？}
    R1 -->|是| C2[L1 Schema加载<br>通过InfoSQLDatabaseTool<br>获取候选表的DDL和样本行]
    R1 -->|否 - 扩展范围| V
    C2 --> P[查询规划<br>分解：表·关联·<br>过滤·聚合<br>通过思维链推理]
    P --> G[SQL生成<br>合成候选查询<br>使用显式列名和<br>行数限制约束]
    G --> CHK[L1复核校验<br>QuerySQLCheckerTool<br>基于缓存的Schema上下文<br>进行交叉验证]
    CHK --> EX[L2数据访问<br>通过QuerySQLDatabaseTool<br>执行查询]
    EX --> ERR{执行<br>出现错误？}
    ERR -->|是| EC[错误上下文注入<br>将错误追加至历史<br>重新审查Schema缺口<br>重载受影响的L1部分]
    EC --> G
    ERR -->|否| RS[响应合成<br>将结果集转换为<br>自然语言回答]
    RS --> OUT([最终回答])

    style Q fill:#EBF5FB,stroke:#2E86C1
    style OUT fill:#E9F7EF,stroke:#27AE60
    style ERR fill:#FDEDEC,stroke:#E74C3C
    style EC fill:#FDEDEC,stroke:#E74C3C
```

**图4.** 查询处理流水线，展示上下文如何随Agent对数据环境理解的深入，从L0逐层加载至L2。

该流水线内嵌的渐进式加载策略具有重要的Token效率特性。对于可通过L0目录信息完全解答的查询（例如列举可用表），不会触发L1加载。对于需要Schema上下文但结果集较小的查询，仅加载L1而L2物化量极少。只有既需要深度Schema理解、又需要处理大型结果集的复杂分析查询，才会产生三层完整加载的成本。在实践中，与在查询初始化时注入完整Schema的基线方案相比，该级联机制可将每次查询的平均上下文窗口消耗降低约40%至60%。

#### 3.6.1 Schema歧义消解

当查询关键词与表名之间的语义匹配存在歧义时——这在业务领域术语与数据库命名规范不一致时尤为常见——Agent利用L1层中的样本行进行歧义消解。例如，关于"营收"的查询可能需要被路由至名为 `txn_line_items` 的表，而非命名更直观的 `revenue` 表。L1层中的样本行揭示了 `txn_line_items` 表中存在货币类型数值，使Agent得以纠正初始的路由决策。这种**数据驱动的歧义消解**无法通过纯Schema检查实现，代表了样本行纳入策略在实践中的关键优势。

#### 3.6.2 自纠错反馈循环

自纠错机制值得特别关注，因为它是相对于静态流水线架构实现+35%至+45%任务解决率提升的主要原因。当 `QuerySQLDatabaseTool` 返回数据库错误时，错误消息作为一等公民的观测结果被追加至Agent的行动-观测历史中。LLM随后在已累积的Schema知识背景下对错误内容进行推理：

- `OperationalError: no such column: X` 观测结果触发对受影响表的 `InfoSQLDatabaseTool` 再次调用，以核实正确的列名；
- 关联谓词中的类型不匹配错误，触发对两个参与表的L1 Schema进行重新审查，以识别正确的类型转换或数据转换操作；
- 查询超时（被解读为过于宽泛的全表扫描），触发基于Schema中已识别的索引列追加更具约束力的 `WHERE` 子句。

上述从错误到Schema缺口再到修复的推理链可以迭代多次。可配置的最大迭代深度（通常为3至5次循环）防止在真正格式错误的查询上产生无限循环。

---

### 3.7 状态更新与上下文管理

LangChain SQL Agent 的上下文管理需应对一个根本性张力：完整性（加载所有潜在相关的上下文）与效率（在固定Token预算内不产生不必要的开销）之间的权衡。框架通过三种互补机制共同化解这一张力。

#### 3.7.1 大规模Schema的渐进式披露

在拥有数千张表的企业环境中，仅在会话初始化时加载所有表的L0目录条目，就可能超出实际的上下文窗口约束。渐进式披露架构通过将表组织为**语义簇**（例如"销售分析"、"库存管理"、"人力资源运营"）来解决这一问题，初始阶段仅加载簇级元数据。当Agent的推理识别出相关簇后，该簇的表目录被展开进入工作上下文，随后按需加载相关表的L1 Schema。
...
graph TD
    ...
    style CO fill:#FDEDEC,stroke:#E74C3C
    style TH fill:#FEF9E7,stroke:#F39C12
```

**图5.** 大规模Schema环境下的渐进式披露工作流。当工作上下文接近压缩阈值（通常为模型上下文窗口的85%）时，触发上下文卸载机制。

上下文卸载在工作上下文达到模型Token预算约85%时触发，通过以简洁的指针摘要替换冗长的旧工具观测结果（例如："*sql_db_schema(orders)：5列表；参见缓存的Schema条目*"）来实现压缩。这确保了模型的固定Token预算始终分配给高信号、即时相关的信息，而非冗余的历史观测。

#### 3.7.2 通过LangGraph检查点进行会话状态更新

LangGraph的检查点系统为Agent执行过程中的**非破坏性状态更新**提供机制。在每次节点跳转时，完整的Agent状态被序列化并持久化至可配置后端（开发环境使用内存，生产环境使用分布式键值存储）。该架构支持三种重要的操作模式：

**增量状态累积。** 每次工具观测结果以追加方式写入状态历史，而非替换先前状态，从而完整保留推理轨迹，用于调试和评估目的。

**有状态的错误恢复。** 进入自纠错循环时，Agent恢复至紧接在失败查询执行之前的状态快照，而非从会话开始处重启。这实现了对故障组件的针对性重新合成，同时保留所有有效的先验上下文。

**人机协同中断。** 工作流可配置为在阶段4后（查询校验完成后、阶段5执行前）暂停，将当前状态——包括提议的SQL查询——序列化至持久存储。人工审核员可通过外部界面检查、修改或拒绝查询，之后恢复执行。Agent加载检查点状态并以审核后的查询继续执行阶段5。

#### 3.7.3 长期记忆更新与知识积累

对于持久化多会话部署，框架支持一套异步记忆更新流水线，将完成会话中的高价值知识提炼并写入长期上下文存储。
...
sequenceDiagram
    ...
    A->>A: 汇总提炼：\n提取高价值信号\n（表映射·领域规则·查询模式）
    A->>LM: 写入提炼后的记忆条目\n（冲突时覆盖，互补时追加）
    Note over LM: 记忆条目被索引\n供未来会话进行\n语义检索
```

**图6.** 长期记忆更新流水线。每次会话结束后，高信号信息被提炼并持久化至外部长期存储，为未来会话预填充已积累的Schema与领域知识。

提炼过程识别三类具有长期价值的信息：（1）**已确认的表映射关系** — 业务术语与数据库表/列名称之间经过验证的关联；（2）**领域约束** — 用户在历史会话中指定的业务规则（例如，"有效作业时间以分钟而非秒为单位计量"）；（3）**成功查询模式** — 产生正确结果的（查询意图，SQL）示例对，作为少样本示例（Few-shot Examples）保留，供后续查询合成使用。

新会话开始时，长期存储针对传入的用户查询进行检索，相关条目作为预加载的L1知识注入会话初始上下文，有效跳过历史上已探索查询域的阶段1和阶段2。这一机制使Agent的冷启动开销随使用次数增加而持续降低，并提升重复访问模式下的查询准确率。

长期存储的冲突解决遵循以下策略：对于事实性条目（例如，由于Schema迁移导致列名变更），采用**覆盖写入策略**；对于非冲突性补充条目（例如，在同一领域簇中发现新表），采用**追加策略**。可选的时间衰减加权评分机制对长期未被访问的条目进行降权处理，防止在长期运行的部署中积累过时的Schema知识。
